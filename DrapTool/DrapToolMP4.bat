...
rem -----------------------
:mp4optimize
%scriptpath%\library\ffmpeg.exe -n -i "%filelocation%" -vcodec libx264 -crf %mp4quality% %outputname%
goto :eof
...
rem -----------------------
:webmoggconvert
%scriptpath%\library\ffmpeg.exe -n -i "%filelocation%" %ENCODER% "%filepath%%outputfolder%\%filewithoutextension%.%filetype%"
goto :eof
